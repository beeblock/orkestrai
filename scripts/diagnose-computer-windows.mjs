...
for (const environment of ['inherited', 'restricted']) {
  for (const stdin of ['pipe', 'ignore']) {
    const result = await new Promise(resolve => {
      const started = Date.now();
      const child = spawn('powershell.exe', ['-NoLogo', '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-EncodedCommand', Buffer.from(script, 'utf16le').toString('base64')], { windowsHide: true, env: environment === 'inherited' ? process.env : restricted, stdio: [stdin, 'pipe', 'pipe'] });
