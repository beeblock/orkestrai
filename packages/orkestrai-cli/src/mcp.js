...
const TOOLS = [
  ...
  { name: 'portal_navigate', description: 'Abre URL no portal identificado por nome unico ou ID.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Optional traceability: when provided, must be an active task assigned to this terminal.' }, nodeId: { type: 'string', description: 'Nome unico ou ID do portal.' }, url: { type: 'string' } }, required: ['nodeId', 'url'] } },
  { name: 'portal_eval', description: 'Disabled for agent safety. Use typed Portal snapshot/click/type/select/extract tools instead; arbitrary scripts cannot inspect authenticated sessions.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Optional traceability: when provided, must be an active task assigned to this terminal.' }, nodeId: { type: 'string', description: 'Nome unico ou ID do portal.' }, js: { type: 'string' } }, required: ['nodeId', 'js'] } },
  { name: 'portal_dom', description: 'Devolve o HTML atual do portal identificado por nome unico ou ID.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Optional traceability: when provided, must be an active task assigned to this terminal.' }, nodeId: { type: 'string', description: 'Nome unico ou ID do portal.' } }, required: ['nodeId'] } },
  { name: 'portal_screenshot', description: 'Captura a tela do portal identificado por nome unico ou ID (base64).', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Optional traceability: when provided, must be an active task assigned to this terminal.' }, nodeId: { type: 'string', description: 'Nome unico ou ID do portal.' } }, required: ['nodeId'] } },
  { name: 'portal_tabs', description: 'Lista, cria, ativa ou fecha abas da sessao persistente do Portal.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Optional traceability: when provided, must be an active task assigned to this terminal.' }, nodeId: { type: 'string' }, operation: { type: 'string', enum: ['list', 'new', 'activate', 'close'] }, url: { type: 'string' }, tabId: { type: 'string' } }, required: ['nodeId', 'operation'] } },
  { name: 'portal_snapshot', description: 'Retorna a arvore semantica interativa do Portal com referencias validas ate a proxima snapshot ou alteracao do elemento: e1, e2 etc. Use antes de click/type/select/upload/download.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Optional traceability: when provided, must be an active task assigned to this terminal.' }, nodeId: { type: 'string' }, interactiveOnly: { type: 'boolean', default: true } }, required: ['nodeId'] } },
  { name: 'portal_click', description: 'Clica em uma referencia semantica obtida por portal_snapshot.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Optional traceability: when provided, must be an active task assigned to this terminal.' }, nodeId: { type: 'string' }, ref: { type: 'string', pattern: '^e\\d+$' }, button: { type: 'string', enum: ['left', 'middle', 'right'], default: 'left' } }, required: ['nodeId', 'ref'] } },
  { name: 'portal_type', description: 'Digita em uma referencia semantica; pode limpar o valor e enviar Enter.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Optional traceability: when provided, must be an active task assigned to this terminal.' }, nodeId: { type: 'string' }, ref: { type: 'string' }, text: { type: 'string' }, clear: { type: 'boolean', default: true }, submit: { type: 'boolean', default: false } }, required: ['nodeId', 'ref', 'text'] } },
  { name: 'portal_select', description: 'Seleciona um ou mais valores em um campo do Portal.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Optional traceability: when provided, must be an active task assigned to this terminal.' }, nodeId: { type: 'string' }, ref: { type: 'string' }, values: { type: 'array', items: { type: 'string' }, minItems: 1 } }, required: ['nodeId', 'ref', 'values'] } },
  { name: 'portal_upload', description: 'Envia arquivos confinados ao workspace para um input file referenciado.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Optional traceability: when provided, must be an active task assigned to this terminal.' }, nodeId: { type: 'string' }, ref: { type: 'string' }, paths: { type: 'array', items: { type: 'string' }, minItems: 1 } }, required: ['nodeId', 'ref', 'paths'] } },
  { name: 'portal_download', description: 'Aciona um download e o confina a pasta configurada dentro do workspace.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Optional traceability: when provided, must be an active task assigned to this terminal.' }, nodeId: { type: 'string' }, ref: { type: 'string' }, filename: { type: 'string' } }, required: ['nodeId', 'ref'] } },
...
for (const [command, description] of Object.entries(VIDEO_TOOL_DESCRIPTIONS)) {
  ...
  if (command === 'brands') { properties.input = BRAND_COMMAND_SCHEMA; required.push('input'); }
  if (command === 'sequences') { properties.input = SEQUENCE_COMMAND_SCHEMA; required.push('input'); }
  if (command === 'recipes') { properties.input = RECIPE_COMMAND_SCHEMA; required.push('input'); }
  if (command === 'characters') {
    properties.input = { type: 'object', additionalProperties: false, properties: {
      ...
      definition: { type: 'object', additionalProperties: false, properties: { name: { type: 'string', maxLength: 80 }, appearance: { type: 'string', maxLength: 8000 }, productionNotes: { type: 'string', maxLength: 8000, description: 'Local production/approval/source notes. Excluded from generation prompts. Keep appearance and voice.style limited to actual visual/vocal direction.' }, images: { type: 'array', maxItems: 12, items: { type: 'string', description: 'Workspace-relative image path generated through the Codex image workflow.' } }, voice: { oneOf: [
...
async function callTool(bridge, findFreePort, selfAgent, name, args = {}) {
  ...
  switch (name) {
    ...
    case 'status': {
      if (!selfAgent && !args.taskId) throw new Error('identidade do agente desconhecida (ORKESTRAI_NODE_ID ausente e nenhuma tarefa atribuída foi informada).');
