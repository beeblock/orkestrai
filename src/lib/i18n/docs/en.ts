...
Header: Authorization = Bearer {{accessToken}}`,
    ...
    {
      ...{"id":"video-workflows","title":"Video workflows with fal.ai","body":"In Canvas, open the Image menu and choose Add video workflow. The same node opens in Workbench. Existing Codex image generation remains unchanged and needs no API key. Video is a separate, opt-in paid fal.ai integration: open Configure access in the node, save an account key in the desktop vault, enable the account and this workspace, authorize external prompts/images, select models, and set USD reservation limits and concurrency. Agents additionally need the workspace agent permission, a live authenticated terminal, and a task assigned to them.","bullets":["Wan 2.7 generates video from text, with 2–15 seconds, 720p/1080p and enumerated aspect ratios. Kling 3 Pro animates a selected first-frame Image node, optionally with an end frame, for 3–15 seconds. Kling inputs must be PNG/JPEG/WebP, at least 300 px per side, up to 10 MB and within a 0.4–2.5 aspect ratio. Only supported controls appear. Kling speech supports English and Chinese; Wan may create background audio automatically.","Select context Notes explicitly; their text becomes part of the outgoing prompt. Select first/last images by node identity. Ordinary collaboration edges do not execute other branches. Save, estimate cost, inspect the base estimate and reservation, then generate. The local reservation is four times the base-unit estimate as headroom, not a guaranteed cap on the provider invoice. Enforce account-level limits on fal.ai as well. Existing workspace security gates can require approval for paid operations.","Runs are persisted before submission. After restart, Orkestrai polls the saved remote job rather than generating again. If the paid submission itself loses its response, the run is marked unconfirmed and is not retried automatically. Check the fal.ai queue/account before taking further action. Cancellation requested is not confirmed cancellation or a refund; a completed result can win that race.","MP4 results are downloaded to the chosen workspace-relative folder with size limits and SHA-256 provenance, then appear as Video nodes with native controls. A failed download can be retried without another generation. Files are not stretched, cropped or re-encoded. Deleting the node does not delete delivered files. Copying a workflow clears account authority and run IDs; selected image/note bindings are remapped. Active paid runs block transfer and deletion.","Keys remain in the desktop secure vault; no key or signed provider URL is stored in node payloads or returned to agents. fal request JSON persistence is disabled and supported outputs request a one-hour lifetime and private CDN ACL. These controls do not guarantee privacy or deletion at every upstream model provider. Only send material you are authorized to share. Files already downloaded into the workspace are retained until you remove them.","Agents use video_workflow_list/read/create/update/preview/run/cancel/retry_download/remove or orkestrai video. List exposes permitted profiles and model capabilities, never credentials. Use the returned revision and preview ID with a stable UUID idempotency key. A queued request is not a delivered video. Do not use raw provider HTTP to bypass workspace policy. Runway direct integration, video editing/timeline, arbitrary ComfyUI JSON and reusable execution subgraphs are not part of this first video adapter."]},
    },
    {
      id: 'image-workflows',
  ...
  useCases: [
    ...
    {
      ...{"id":"creative-video-workflow","title":"Turn a brief into a video","body":"Create a vertical five-second text-to-video draft. This action does not contact fal.ai or spend credit. Open Configure access in the node. Save the fal.ai key in the desktop vault, enable the account and workspace, allow external media, select Wan 2.7 and set reservation limits. Allow agents only when you want assigned agents to submit paid video jobs. Select the account, edit the direction, save and estimate. Review the reservation and any Security gate before Generate video. For image-to-video, choose Kling and select an existing Image node as the first frame. Wait for Completed, play the Video node and download the original. Check the output under generated/videos. Retry download reuses the remote result without regenerating. Your existing Codex image flows are unchanged.","tags":["fal.ai","Canvas","MP4"]},
    },
    {
  ...
  changelog: [
    {"date":"Unreleased","title":"Orkestrai 0.32.0: native video workflows","summary":"Opt-in fal.ai video generation on the existing Canvas and Workbench.","items":["Generate videos with Wan 2.7 from text or Kling 3 Pro from selected Image nodes and context Notes. Model-specific controls and reusable MP4 results leave Codex subscription image generation unchanged.","Configure vault credentials, workspace and model permissions, agent access, outgoing-data consent, estimates, budget reservations and concurrency under existing security gates.","Recover persistent jobs without duplicate paid submissions, cancel tracked work and retry downloads into the project. Manual UI, CLI/MCP, documentation and guided onboarding share the same workflow.","Require a protected operating-system credential store and preserve existing credentials when the vault file is unreadable or malformed."]},
