...
Header: Authorization = Bearer {{accessToken}}`,
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Add a Codex Creative Director, a Visual Brief note, and a Generate images workflow. Use an authenticated Codex account or plan with ImageGen available; no OpenAI API key is needed. You can configure the graph manually, or ask the connected Creative Director to create the workflow, write or revise its direction, add and order workspace references, request up to ten results, execute the complete run, and return every validated output to the canvas. The agent uses its built-in image_gen.imagegen tool with referenced_image_paths, copies each result to its assigned destination, and completes one traceable run. Approved poses can feed the next branch without losing prompt context or provenance. A non-Codex leader can coordinate the work but must connect or delegate execution to Codex.',
