...
Header: Authorization = Bearer {{accessToken}}`,
    ...
    },
    {
      ...
      body: `Add Code Graph from the Canvas toolbar to index the approved repositories of the workspace without executing project code, build scripts, plugins, or configuration. The same persistent node opens in Workbench and visualizes modules, imports, classes, interfaces, functions, methods, calls, instantiation, inheritance, and implementations for TypeScript, JavaScript, Svelte, and PHP. Choose one repository or the whole workspace, search names, paths, signatures, and nearby documentation, then inspect incoming, outgoing, or bidirectional relationships up to a bounded depth. Open Changes to combine the current Git working tree with every active Floor: changed files are mapped to direct and transitively affected symbols, likely test files, and shared-impact conflicts before review or landing. The actions beside each scope create a bounded Kanban handoff; the primary working tree can also create a Review Center review tied to its current Git revision. Source files remain authoritative: Orkestrai stores hashes, spans, bounded metadata, and relationships in its own versioned SQLite graph adapter, swaps revisions atomically, and marks the index stale when supported files change. Additional sibling repositories explicitly authorized in Edit workspace become separate graph projects. Agents query that exact visible graph through code_graph_status/index/search/symbol/neighbors/changes/handoff or the equivalent orkestrai graph commands; arbitrary SQL and Cypher are never exposed. SQLite is the local-first default behind CodeGraphStore, while benchmark gates keep a future Memgraph adapter possible without changing the UI or agent contract.`,
    },
    {
      id: 'cli',
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Start the guided use case to add one Code Graph node and index every repository explicitly approved for the workspace. Search for a service, component, route, or method, select the result, and switch between callers and dependencies before opening the relevant source. Open Changes to inspect the current working tree and active Floors, including affected symbols, likely tests, and cross-Floor conflicts, then use the scope actions to create a Review Center review or Kanban task. A leader or specialist uses the same bounded graph and handoff through MCP without loading whole repositories into the conversation.',
