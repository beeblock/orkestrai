...
Header: Authorization = Bearer {{accessToken}}`,
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Start the guided use case to add one Code Graph node and index every repository explicitly approved for the workspace. Search a service, component, route, or method, explain a relationship with its provenance, and open the exact symbol in Monaco. Overlay live agents, tasks, and Floors to expose ownership conflicts, then build a reviewed context package within a chosen token budget for the leader, an agent, Council, or a traceable task. Compare revisions and save the project, view, filters, selection, camera, and open source as a named investigation. Contracts, Quality, Semantic, Runtime, and Changes remain available in the same graph, and agents use the same bounded operations through MCP without loading whole repositories into the conversation.',
