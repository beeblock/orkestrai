...
Header: Authorization = Bearer {{accessToken}}`,
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Start the guided use case to add one Code Graph node and index every repository explicitly approved for the workspace. Search for a service, component, route, or method, select the result, and switch between callers and dependencies before opening the relevant source. Open Changes to inspect the current working tree and active Floors, including affected symbols, likely tests, and cross-Floor conflicts. A leader or specialist can use the same bounded graph through MCP to prepare an architecture explanation or an impact-aware task without loading whole repositories into the conversation.',
