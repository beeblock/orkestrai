...
Header: Authorization = Bearer {{accessToken}}`,
  ...
  changelog: [
    {"date":"Next release","title":"Orkestrai: a new Canvas and Workbench experience","summary":"A cleaner interface with one-click actions, smooth motion and comfortable reading.","items":["Menus and dialogs open smoothly and close faster; surfaces use theme-aware elevation instead of hard borders; every control shares one focus ring; UI text never drops below 11px and all motion respects the system reduced-motion setting.","Redesigned Canvas sidebar: calmer rows, a clear selected state, row actions that appear on hover or focus, a trash icon for deletion and inline folder creation.","Canvas tools are grouped by purpose; the minimap and zoom controls no longer sit under the tool bar; canvas messages appear above the tool bar and can be dismissed.","One-click starting points: create or reopen a workspace when none is open, and add an agent, note, task board, file tree or team preset when a canvas is empty.","Refined node frames, headers, actions, selection and connection handles; rename a node from the keyboard with Enter or F2.","Notifications and dialogs follow every app theme, and the dictation button stays quiet until it is pointed at, focused or used.","Shows “1 selected” when a single node is selected.","Fixes surfaces and text in Computer, Integrations, Tool Workshop, Storyboard, Sequence and Knowledge that used missing theme colors.","Terminal: voice controls gather in one compact group that appears on hover or focus and stays visible while recording; the quick prompt highlights while focused and its send button lights up when there is text.","Notes: pick the color from a single button instead of six dots in the header; the text uses a reading font and the frame follows the note color.","Tasks: move cards with the Move to menu or Alt+arrow keys, edit with Enter and see secondary actions only on hover or focus; columns, counts and drop zones are clearer.","Workbench: tree with 30px rows, neutral selection with an indicator, a labeled Hubs group, hover or focus row actions and mono counts; tabs mark the active one with an accent line and keep its close button visible; the header shows a readable trail and empty states offer the next action.","Workbench usage footer with mini meters per window, threshold colors and aligned percentages.","Control Center: activity reads in plain language (for example “Status updated · Maestro”), translated categories, prominent metrics and a sliding Activity/Communications switch.","Canvas side panels (Floors, Ports, Roles and Presets) share one frame: an icon title with description, the close button always in the same place and a smooth entrance; calmer preset cards highlight the main action on hover.","Settings: the navigation marks the visible section; minimap and zoom controls become switches; font size, padding and editor font use sliders with the value shown; new node sizes show their unit in the field; canvas connections and Workbench tabs use segmented selectors.","Provider Center: a filter with counts, neutral elevated cards (status lives in the badge), Open Canvas as a row action and a profile form with clear focus.","How to use: the index marks the visible section and long use cases start collapsed with Read more. Skills & MCPs: a stable width, elevated sections and app-standard tabs.","Command palettes: dedicated icons for each command, rows without repeated labels, a larger search field, the highlighted item stays in view while navigating and a footer lists keyboard shortcuts.","Welcome flow: the language in use is marked Current and receives focus, instead of the first option looking selected.","Drag any creation tool from the dock onto the canvas to create the node exactly where you drop it; the canvas highlights the drop area and tool tips explain click-to-draw or drag.","Workbench: drag sidebar items straight into any pane to open them there.","Remove the blue resize outline that covered the selection ring of a selected node.","Rework the file tree, diff, editor, Git and portal nodes: a real search field, colored Git status chips with readable names, file actions on hover or focus, loading placeholders, empty and error states with a next step, a commit field that lights up when ready, a lighter portal header with settings on its control chip, and sliders for device dimensions.","Let the Git file list scroll instead of being cut off, stop showing editor confirmations in red and stop flashing \"empty folder\" before a file tree loads.","Keep compact interface text at its intended size when a component also sets a text color, instead of falling back to a larger default.","Rework the creative nodes: drop an image straight onto an empty image node, see the image executor's readiness and why Generate is unavailable, reorder sequence clips by dragging, pick storyboard ratios and shape styles with segmented controls, switches and sliders, and get clear loading, empty and error states with a recovery action.","Give shapes a floating toolbar that frees their resize handles, and give groups a dashed frame, a raised label and keyboard rename with Enter or F2.","Rework the automation nodes: set loop rounds with a slider and file writing with a switch next to a clear Run button, reorder flow steps by dragging or with the arrow keys, adjust the usage threshold with a slider and the window with a segmented control, and read usage as meters with reset times.","Calm the API client: theme colors throughout, a request list whose actions appear on hover, empty states with an Add request action, segmented toggles, status chips with icons and drag-to-order requests in the runner.","Show \"no usage data reported\" in a neutral tone instead of green, and make the empty API client's Add request button create a request at the top level.","End truncated titles and descriptions with an ellipsis instead of wrapping them onto a second line.","Rework the code graph, Second Brain, document, device, computer and tool workshop views: a one-row code graph toolbar with Index as the single primary action, stats that appear once indexed, a slider for depth and token budget, source rows with kind icons and tags, document and device empty states that offer the next step, computer settings grouped into cards with rarely used sections folded, and tool status chips with readable executor names.","Make the code graph's Index button clickable in its empty state and keep the device tools panel from being clipped in narrow nodes.","Rework the Workbench hubs: the Review Center gets calm file rows with stage and discard on hover, an anchored comment popover and clear loading and empty states; Workstreams, Annotations and Attention get segmented filters with counts and a proper search field; Huddles show a live chip, clickable participant rows and message avatars; Automations reveal row actions on hover and show recipes as cards; the concurrent runs limit is a slider; and the file explorer and file toolbar match the Workbench sidebar.","Translate recipe categories and security audit events, give vault fields visible labels, keep a long revision hash inside its Annotations column and show review risks with a warning icon instead of a check.","Show the Huddles icon in open Workbench items and tabs, and name Huddles and Workstreams in the pane header instead of their internal type.","Give every dialog the same anatomy: a title with a one-line description, grouped fields with hints, errors in one consistent block and a footer with the primary action last. Runtimes, modes and platforms use segmented controls; agent limits use a stepper, a slider and a minutes field; workspace settings are split into sections; and themes are picked from preview cards with color swatches.","Show update downloads as a floating progress pill, announce tour steps to screen readers and tuck rarely used sharing options under Advanced.","Highlight the active scope tab in the terminal commands dialog, which never showed as selected.","Keep the remote workspace responsive when the shared workspace has a huddle, instead of freezing its tabs.","Guide remote connections step by step: a status pill, labeled invite and device fields with helper text, relay settings under Advanced, a recovery hint for every failure and a How it works guide that follows your progress. The connected workspace uses calm sidebar rows, state-colored pills, empty states and labeled design fields, and the Share button says whether the workspace is shared and who is waiting.","Rework the design editor chrome: grouped top bar and tool rail with shortcut hints, a pinned selection bar, drag-to-reorder layer and page rows, layer visibility and lock on hover, 28px inspector rows aligned on one label column, segmented controls for sizing, layout, alignment and text styles, an opacity slider, rarely used limits and text options behind toggles, real boolean and alignment buttons for multiple selections, swatch pickers for variable and keyframe colors, and empty states that point to the next step in every panel.","Show readable prototype trigger names instead of internal values, keep the design selection toolbars visible while the canvas scrolls and stop the fill opacity value from being cut off.","The empty-workspace panel in the middle of the canvas accepts dropped tools, files and characters.","Remaining small counts, paths and badges now use the 11px minimum, and count pills on tinted backgrounds are readable in every theme.","Icon-only segmented options show a tooltip, and selected permission, grant and huddle target chips are highlighted again.","The collection runner's form scrolls inside its dialog, so requests further down are no longer hidden under the footer."]},
