...
Header: Authorization = Bearer {{accessToken}}`,
    ...
    },
    {
      ...
      body: `Add Tool Workshop from the Canvas toolbar, open the same node full-screen from Workbench Tools, or select Tools inside Automations. Build a versioned workspace tool from a connected integration, a bounded HTTP request, a deterministic transform, or a workspace command with an argument array and no shell expansion. Each manifest declares supported JSON input/output schemas, capabilities, timeout, output limit, and test fixtures. A dry run validates the current draft without performing its effect. Only the workspace owner publishes an immutable revision; agents with an assigned active task can propose or update drafts and execute the approved version through tool_list, tool_propose, tool_update, and tool_execute, but cannot activate their own code. A later draft never replaces the live revision until explicit publication, and rollback creates a new reviewable draft. Credentials use only SecretRefs from Automations -> Security and must be bound to Tool Workshop, tool:<slug>, and the exact host or workspace-command destination before publication. The trusted executor resolves values only into an HTTP header or protected command stdin; raw credentials are rejected from manifests and run inputs, while persisted inputs, outputs, errors, Control Center activity, and the hash-chained audit remain redacted. Give automations a stable idempotency key so a recovered run cannot repeat an external effect. Existing Gmail, Slack, Telegram, WhatsApp, GitHub, Portal, Computer, PDF, and workspace-file flows can be composed as tools without creating another graph model.`,
    },
    {
      id: 'computer-node',
      title: 'Computer node and bounded desktop control',
      body: `Add Computer from the Canvas toolbar or Workbench Tools. The node starts disabled and inventories the platform, displays, visible applications, windows, accessibility, and screen-capture permission. Grant the operating-system permissions yourself, allow only the required application and display ids, and then enable control. Agents must have an active assigned Kanban task and use typed computer_inspect/focus/click/type/type_secret/shortcut/screenshot/wait calls with stable idempotency keys; the same application ids must also be inside the workspace Security standing grant. Agent clicks and captures stay inside an allowed window, while manual screen-wide actions require every affected display to be selected. A login SecretRef must be bound to computer.type_secret and the exact app id; its value is resolved only inside the executor and travels to macOS or Windows through a protected stdin channel instead of process arguments. The current Linux X11 adapter refuses secret typing; use an API integration or Managed Portal SecretRef there. Screenshots are confined to .orkestrai/computer/evidence with bounded retention and can be used as a click map from the visible node. Control Center and the hash-chained audit store actor, operation, target, outcome, and safe evidence metadata; typed content is represented only by its length and digest. macOS uses native Accessibility and Screen Recording permission, Windows uses the interactive desktop, and Linux currently supports non-secret X11 control with xdotool/wmctrl plus gnome-screenshot or scrot; unsupported Wayland sessions fail explicitly.`,
    },
    {
      id: 'portal',
      title: "Portal (the agents' browser)",
      body: `The Portal node is an embedded browser. Give each Portal a persistent name with the pencil in its header; its address lives in the separate navigation bar. Agents list every Portal in the workspace with name, URL, id, and connection state, then target it by unique name or id: orkestrai portal <name-or-nodeId> navigate (open URL), eval (run JS on the page), dom (read the HTML), screenshot. An unconnected Portal still exists and should be reused; creating an additional Portal requires explicit intent. Connected to an agent, it becomes the agent's eyes. Use it to test the app the team is building (point the portal at the dev server) or to research the web. In the desktop app, normal links requesting a foreground or background tab create another Portal node in the same Canvas. True login and payment pop-ups remain in a sandboxed Orkestrai window instead of the system browser, preserving window.opener and the shared Portal session. Persistent cookies and web storage are flushed to disk, and the node restores its last navigated URL after restart; sites may still intentionally use session-only cookies that expire on close. Full automation runs in Electron; in a regular browser the portal is viewer-only. The phone icon in the Portal header opens a responsiveness toolbar, similar to a browser's device toolbar: pick a device preset (iPhone, Pixel, iPad, laptop, desktop) or type an exact width/height, rotate orientation, or turn it off to go back to filling the node. The page's real viewport changes to that exact size — the same as resizing a real window — so its own responsive CSS reacts normally; if the emulated size is larger than the node, the Portal scrolls to it rather than shrinking or distorting the page.`,
    },
    {
      id: 'portal-new-tabs',
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Add one Computer node, grant the required operating-system permissions, allow only the target app and displays, and enable the same app id under Automations -> Security. Connect a persistent agent with an active Kanban task. It can inspect visible windows, focus the approved app, click normalized or absolute coordinates, type, use bounded shortcuts, wait for a window state, and capture evidence. Every brokered action is idempotent and audited, while typed text and credentials remain out of the records. Prefer native Integration Center or Managed Portal operations when available; use Computer for installed tools that expose neither API nor browser workflow.',
