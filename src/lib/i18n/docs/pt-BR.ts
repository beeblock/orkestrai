...
Header: Authorization = Bearer {{accessToken}}`,
    ...
    },
    {
      ...
      body: `Abra o menu Imagem na barra do Canvas e escolha Gerar imagens. O fluxo faz parte do canvas existente: conecte Notas para paleta, regras do personagem, direção de arte, restrições ou contexto da campanha; conecte até cinco referências ordenadas de Imagem PNG, JPEG ou WebP; e conecte um agente Codex ativo como executor. É necessária uma conta ou assinatura do Codex autenticada com ImageGen disponível; nenhuma chave de API da OpenAI é necessária. Miniaturas numeradas mostram a ordem exata. Digite o prompt, escolha de um a dez outputs, opcionalmente peça fundo PNG realmente transparente e defina uma pasta relativa no workspace (generated/images por padrão) com um prefixo seguro. Gerar delega um único run lógico à sessão do Codex. O agente produz e valida cada output separadamente; quando alpha real é obrigatório e o ImageGen devolve PNG opaco ou quadriculado desenhado, ele edita automaticamente aquele resultado para remover o fundo e tenta até três vezes antes de interromper. O Orkestrai nunca pede nem armazena chave de API de imagem, nunca chama diretamente um endpoint do provider e não inventa controles de modelo, qualidade, tamanho ou formato que a tool nativa não expõe. Um Codex conectado tem controle tipado completo desse fluxo visível por image_workflow_*: pode criar e configurar um rascunho sem executá-lo, conectar, desconectar ou reordenar Notas e Imagens, adicionar arquivos de referência do workspace, assumir e executar a geração, validar cada output, concluir ou cancelar e remover o fluxo. O executor copia cada resultado nativo para o destino pré-alocado no workspace e chama image_workflow_validate antes de image_workflow_complete. O Orkestrai aceita somente aquele Codex conectado e aqueles caminhos exatos, decodifica cada PNG com limites seguros de dimensão e verifica pixels realmente transparentes quando solicitado antes de criar nodes de Imagem conectados com hash dos inputs, fluxo de origem, id da execução, índice do output, duração e histórico limitado. Outputs podem alimentar outro fluxo como referências. Cancelar invalida o run ativo para impedir que uma conclusão atrasada seja materializada; execuções abandonadas também expiram em tempo limitado. Canvas, Workbench, CLI e MCP operam o mesmo grafo persistido sem um estado paralelo de automação.`,
      bullets: [
        ...
        'A remoção do fundo é sempre outra edição nativa de image_gen.imagegen usando somente o output rejeitado como referência e o prompt dedicado do validador. Com várias referências de imagem, o ImageGen primeiro compõe o primeiro plano sobre um fundo branco uniforme e remove esse fundo em uma edição nativa separada. O feedback de máquina identifica depois qualquer fundo gravado como pixels opacos e escala o prompt de recorte se a correção continuar opaca. Python, Pillow, ImageMagick, ffmpeg, remove-bg, máscaras geradas e processamento local de pixels são proibidos.',
      ],
    },
    {
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Inicie o caso de uso guiado para adicionar um nó Grafo de Código e indexar todos os repositórios explicitamente aprovados para o workspace. Pesquise um serviço, componente, rota ou método, selecione o resultado e alterne entre quem chama e suas dependências antes de abrir o código relevante. Abra Mudanças para inspecionar o working tree atual e os Floors ativos, incluindo símbolos afetados, testes prováveis e conflitos entre Floors. Um líder ou especialista usa o mesmo grafo limitado pelo MCP para preparar uma explicação de arquitetura ou uma tarefa consciente do impacto sem carregar repositórios inteiros na conversa.',
