...
Header: Authorization = Bearer {{accessToken}}`,
    ...
    },
    {
      ...
      body: `Adicione Tool Workshop pela barra do Canvas, abra o mesmo node em tela cheia por Ferramentas no Workbench ou selecione Ferramentas dentro de Automações. Crie uma ferramenta versionada do workspace a partir de uma integração conectada, uma chamada HTTP limitada, uma transformação determinística ou um comando do workspace com array de argumentos e sem expansão de shell. Cada manifesto declara schemas JSON suportados de entrada e saída, capacidades, timeout, limite de saída e fixtures de teste. O teste seco valida o rascunho atual sem executar o efeito. Somente o dono do workspace publica uma revisão imutável; agentes com tarefa ativa atribuída podem propor ou atualizar rascunhos e executar a versão aprovada por tool_list, tool_propose, tool_update e tool_execute, mas não podem ativar o próprio código. Um rascunho posterior nunca substitui a revisão ativa sem publicação explícita, e rollback cria outro rascunho revisável. Credenciais usam apenas SecretRefs de Automações -> Segurança e precisam estar vinculadas ao Tool Workshop, a tool:<slug> e ao host exato ou ao destino workspace-command antes da publicação. O executor confiável resolve valores somente num header HTTP ou no stdin protegido do comando; credenciais brutas são recusadas no manifesto e nos inputs, enquanto inputs, outputs, erros, atividade da Central de Controle e auditoria encadeada persistem sanitizados. Use uma chave de idempotência estável nas automações para que a recuperação não repita um efeito externo. Fluxos existentes de Gmail, Slack, Telegram, WhatsApp, GitHub, Portal, Computador, PDF e arquivos do workspace podem ser compostos como ferramentas sem criar outro modelo de grafo.`,
    },
    {
      id: 'computer-node',
      title: 'Nó Computador e controle limitado do desktop',
      body: `Adicione Computador pela barra do Canvas ou por Ferramentas no Workbench. O nó começa desativado e identifica plataforma, monitores, aplicativos visíveis, janelas, permissão de acessibilidade e captura de tela. Conceda pessoalmente as permissões do sistema operacional, permita somente os ids de aplicativo e monitor necessários e então ative o controle. Agentes precisam de uma tarefa Kanban ativa atribuída e usam chamadas tipadas computer_inspect/focus/click/type/type_secret/shortcut/screenshot/wait com chaves de idempotência estáveis; os mesmos ids de aplicativo também precisam estar na política permanente de Segurança do workspace. Cliques e capturas de agentes ficam dentro de uma janela permitida; ações manuais de tela ampla exigem selecionar cada monitor afetado. Uma SecretRef de login precisa estar vinculada a computer.type_secret e ao id exato do app; seu valor só é resolvido dentro do executor e chega ao macOS ou Windows por um canal stdin protegido, sem argumentos de processo. O adaptador Linux X11 atual recusa digitação de segredos; nele, use uma integração de API ou SecretRef do Portal Gerenciado. Capturas ficam confinadas em .orkestrai/computer/evidence com retenção limitada e servem como mapa de clique no nó visível. A Central de Controle e a auditoria encadeada guardam ator, operação, alvo, resultado e metadados seguros da evidência; o conteúdo digitado aparece somente como tamanho e digest. O macOS usa Acessibilidade e Gravação de Tela nativas, o Windows usa o desktop interativo e o Linux atualmente suporta controle X11 sem segredos com xdotool/wmctrl e gnome-screenshot ou scrot; sessões Wayland incompatíveis falham de forma explícita.`,
    },
    {
      id: 'portal',
      title: 'Portal (browser dos agentes)',
      body: `O nó Portal é um navegador embutido. Dê a cada Portal um nome persistente pelo lápis no cabeçalho; o endereço fica na barra de navegação separada. Os agentes listam todos os Portais do workspace com nome, URL, id e estado de conexão e escolhem pelo nome único ou id: orkestrai portal <nome-ou-nodeId> navigate (abrir URL), eval (rodar JS na página), dom (ler o HTML), screenshot. Um Portal não conectado ainda existe e deve ser reutilizado; criar outro exige intenção explícita. Conectado a um agente, ele vira os olhos do agente. Use para testar a aplicação que o time está construindo (aponte o portal para o dev server) ou pesquisar na web. No desktop, links comuns que pedem uma aba em primeiro ou segundo plano criam outro nó Portal no mesmo Canvas. Pop-ups reais de login e pagamento continuam numa janela isolada do Orkestrai, não no browser do sistema, preservando window.opener e a mesma sessão. Cookies persistentes e web storage são gravados em disco, e o nó restaura a última URL navegada após reiniciar; sites ainda podem usar cookies somente de sessão que expiram ao fechar. A automação completa roda no Electron; no browser comum o portal é só visualizador. O ícone de celular no cabeçalho do Portal abre uma barra de responsividade, parecida com o device toolbar de um navegador: escolha um dispositivo (iPhone, Pixel, iPad, laptop, desktop) ou digite largura/altura exatas, gire a orientação, ou desligue pra voltar a preencher o nó. O viewport real da página muda pro tamanho escolhido — igual redimensionar uma janela de verdade — então o CSS responsivo dela reage normalmente; se o tamanho emulado for maior que o nó, o Portal rola até lá em vez de encolher ou distorcer a página.`,
    },
    {
      id: 'portal-new-tabs',
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Adicione um nó Computador, conceda as permissões necessárias do sistema, permita somente o app e os monitores de destino e ative o mesmo id de app em Automações -> Segurança. Conecte um agente Persistente com uma tarefa Kanban ativa. Ele pode inspecionar janelas, focar o app permitido, clicar em coordenadas normalizadas ou absolutas, digitar, usar atalhos limitados, aguardar o estado de uma janela e capturar evidências. Toda ação intermediada é idempotente e auditada, sem guardar o texto digitado nem credenciais. Prefira a Central de integrações ou o Portal gerenciado quando houver suporte; use Computador para ferramentas instaladas sem API nem fluxo web.',
