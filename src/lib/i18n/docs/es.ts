...
Header: Authorization = Bearer {{accessToken}}`,
    ...
    },
    {
      ...
      "title": "Observar cambios en apps nativas sin consultar continuamente a la IA",
      "body": "El escritorio macOS incorpora el SDK oficial Cua en el proceso principal de Electron, con permisos de Orkestrai y sin otra app Cua ni daemon. Las lecturas siguen vinculadas a la ventana; las respuestas con foco autorizado verifican el borrador completo y Enviar por separado. Las respuestas de una línea usan inserción directa del SDK, sin cambiar al atajo de pegar a partir de 64 caracteres. Los borradores multilínea requieren pegado de texto plano verificado, conservando formatos compatibles del portapapeles y nuevas copias del usuario; nunca se envía una tecla literal ni un borrador parcial. Los eventos recuperados exigen explícitamente la solicitud y el texto originales, para que el agente no redacte otra respuesta para el mismo envío. SecretRefs nunca usan esta ruta. La verificación exacta del selector de archivos permanece en un adaptador especializado; operaciones no confirmadas nunca activan una entrada alternativa automática. La inspección del panel Computadora y el monitor automático comparten lecturas en curso en vez de rechazarse como ocupados. Una acción protegida puede esperar una lectura, pero las acciones simultáneas y los reintentos inciertos siguen bloqueados. La jerarquía AX delimitada existente define el alcance de la conversación; la proyección parcial del SDK solo vincula tokens actuales de entrada, nunca demuestra una conversación completa. Tras un timeout, los nuevos trabajos solo continúan cuando el SDK confirma el cierre de las operaciones ya admitidas. Si el cierre falla, se mantiene el bloqueo; las solicitudes pendientes y los envíos inciertos nunca se repiten.\n\nElige Texto nativo, imagen como alternativa en el nodo Computadora existente. En macOS y Windows, el modo automático lee la ventana autorizada en primer plano mediante accesibilidad nativa sin producir PNG. Dos observaciones estables de texto despiertan al operador asignado con una diferencia limitada, sin otra captura ni releer todo el tablero o historial. El intervalo local predeterminado es de un segundo y la pausa entre avisos es de dos segundos; el usuario conserva el control de los valores. El panel muestra la fuente real y la duración de la última comprobación local. Los árboles nativos no disponibles o incompletos utilizan comprobaciones visuales limitadas, separadas por al menos tres segundos. La región y el umbral de píxeles solo afectan a la alternativa visual; la lectura nativa cubre la ventana autorizada. Linux utiliza actualmente la vía visual. El tiempo de detección local no garantiza el tiempo de respuesta del proveedor.\n\nAutoriza la aplicación, activa Seguridad Delimitada con las capacidades computer y agent, crea una tarea activa asignada al operador y selecciona una automatización Manual habilitada que envíe un prompt al mismo operador. Selecciona la ventana exacta, automatización y tarea, después activa Monitorizar. Permitir configuración por el agente es una autorización independiente para computer_watch. No programes además consultas periódicas al modelo. El contenido igual no despierta al modelo. La observación espera el primer plano sin robar el foco. Pausar, descargar, revocar la aplicación o completar/reasignar la tarea detiene futuros trabajos. La observación genérica comienza una nueva referencia al reiniciar. Respuestas en conversaciones recuperan la cola persistente de mensajes observados; consulta ese caso de uso para retención y agrupación.\n\nLos agentes usan computer_read para el texto y los controles actuales, después computer_interact para rellenar o pulsar un elemento nativo exacto. La operación vuelve a comprobar role/name/value esperados y las condiciones de conversación o documento inmediatamente antes de actuar, devolviendo texto actual después. Rellenar exige el valor existente; nunca sobrescribas un borrador humano. Antes de enviar, incluye condiciones para el destinatario exacto y el borrador completo y declara external_publication. Los controles desconocidos, modificados o ambiguos bloquean la acción. Comprueba el resultado real: pulsar Enviar no demuestra entrega. No hacen falta capturas repetidas mientras un gate espera aprobación. Los controles genéricos mantienen los gates de publicación. Para respuestas automáticas usa una autorización de Respuestas en conversaciones aprobada por el usuario y computer_reply, descritos en el caso de uso correspondiente; nunca lo sustituyas por permiso de publicación para toda la app. El contenido de pantalla es dato no fiable, no instrucciones. Las lecturas nativas ocultan controles de contraseña; las capturas aún pueden contener secretos visibles. Las diferencias de texto caducan de la memoria local de entrega tras dos minutos; la auditoría conserva conteos y hashes, no el texto de la conversación. Las transcripciones de proveedores tienen retención independiente.\n\nEn macOS, las lecturas de la ventana seleccionada usan API nativas públicas sin llamadas repetidas a System Events. El proceso con canales privados termina tras 30 segundos inactivo y se recicla entre solicitudes confirmadas después de 256 operaciones o cinco minutos; cerrarlo cancela comandos pendientes sin repetirlos. Otra aplicación puede seguir siendo observada durante una operación dirigida a un proceso. Las lecturas del mismo proceso y la entrada global siguen siendo exclusivas. Una observación rápida o Automático habilitado no demuestran soporte de escritura o Enviar en segundo plano; una entrada no compatible no autoriza un cambio automático de foco.\n\nEn macOS, abre Computadora > Respuestas de conversación y activa Foco temporal para enviar para el contacto exacto. Está desactivado por defecto y solo el usuario puede cambiarlo. computer_reply y computer_send pueden traer la ventana autorizada al frente durante la composición y el envío protegidos. La escritura, los clics recientes o modificadores pulsados aplazan la entrada. Se restauran la aplicación y la ventana anteriores, salvo que el usuario haya cambiado el foco; no se adivinan destinos cerrados o reemplazados. Un fallo de restauración se audita sin repetir el mensaje. La observación continúa en segundo plano. Esta autorización cubre el envío de texto, no los diálogos de archivos ni el control irrestricto del escritorio; siguen vigentes los permisos, las aprobaciones de publicación y la recuperación de envíos inciertos.",
      "bullets": [
    ...
    },
    {
      ...
      bullets: ['Envía el texto completo con una llamada computer_type. En macOS, los eventos Unicode nativos conservan acentos y emoji sin usar el portapapeles. Comprueba el texto y el destinatario antes de enviar; un comando de entrada exitoso no demuestra la entrega.', 'Las acciones vinculadas a una ventana inspeccionan el proceso de destino en vez de enumerar todas las aplicaciones repetidamente. Las comprobaciones actuales de foco e identidad siguen siendo obligatorias; un cambio en el escritorio o un envío incierto exige otra observación, no una repetición ciega.'],
  ...
  changelog: [
    {"date":"23 de septiembre de 2026 · 0.33.0","title":"Orkestrai 0.33.0: más proveedores de vídeo","summary":"Usa fal.ai, BytePlus ModelArk o Higgsfield con cuentas, contratos y presupuestos separados.","items":["Añadir BytePlus ModelArk e Higgsfield como proveedores explícitos de vídeo junto a fal.ai, con contratos propios, cuentas en la bóveda y cola persistente. Se conservan los flujos fal y de imágenes Codex.","Exponer controles coherentes de proveedor/cuenta/modelo, tarifas públicas fechadas y presupuestos configurados de Higgsfield. Conservar referencias obligatorias al cambiar proveedor explícitamente y bloquear credenciales cruzadas o reintentos de pago duplicados.","Documentar límites de subida, cancelación y referencias en los tres idiomas; compartir contratos con agentes, storyboards y onboarding guiado.","Detener las conexiones elásticas al estabilizarse sin reiniciar su propia animación. Limitar la física sin convergencia, reutilizar índices de conexiones y aplazar el pintado de imágenes y flujos fuera de pantalla sin descargar agentes ni modificar archivos originales.","Mostrar referencias de vídeo con nombres y vistas previas para nodos del canvas y archivos del proyecto. Navegar por carpetas, ampliar la vista previa y mantener rutas/mapeos de la API en detalles técnicos sin cambiar vínculos de agentes. Alinear los controles de cuenta y modelo."]},
    {"date":"19 de septiembre de 2026 · 0.32.0","title":"Orkestrai 0.32.0: flujos nativos de vídeo","summary":"Generación opcional de vídeos con fal.ai en el Canvas y Workbench existentes.","items":["Esperar al campo de mensajes de Claude antes de entregar roles, tareas iniciales o Enter automático. La confirmación de confianza de la carpeta queda bajo control del usuario, sin seleccionar No, exit por accidente; un tiempo de espera agotado no se registra como entrega.","Separa las notas de producción y la procedencia de aprobación/versión del prompt; distingue instrucciones de personaje y diálogo sin modificar referencias congeladas.","Muestra descripciones, ejemplos y valores del proveedor a usuarios y agentes; edita parámetros anidados, identifica incompatibilidades al cambiar de modelo y compara tarifas base de la cuenta antes de estimar la ejecución.","Aislar los formularios creativos por diálogo para que restaurar varios nodos no reutilice identificadores de personaje, cuenta, permisos, marca o receta. Mantener el desplazamiento de los ajustes de vídeo con cuenta, pestañas y acciones fijas, también en ventanas compactas.","Mantener los menús de modelos y referencias dentro de la ventana, incluidos los campos cerca del pie de un diálogo.","Reservar las entradas de imagen y voz del personaje al adjuntar medios de la escena, ofreciendo las siguientes entradas libres sin conflictos con identidades aprobadas.","Indicar el tamaño exacto de las respuestas de medios del workspace para que las vistas previas de voz determinen la duración sin esperar al final de la reproducción.","Cerrar la confirmación del codificador de vídeo tras aprobarlo para que la descarga completada no deje el editor de secuencia bloqueado por una superposición.","Busca todo el catálogo de video de fal.ai, incluyendo Seedance 2.5, con contratos específicos, parámetros anidados, múltiples referencias de imagen/video/audio y permisos por workspace. Los borradores Wan/Kling y flujos de imágenes Codex siguen siendo compatibles.","Configura credenciales en la bóveda, permisos por workspace y modelo, acceso de agentes, envío de datos, estimaciones, reservas de presupuesto y concurrencia bajo los controles de seguridad existentes.","Recupera ejecuciones persistentes sin duplicar generaciones de pago, solicita cancelación y repite descargas al proyecto. UI manual, CLI/MCP, documentación y tour guiado comparten el mismo flujo. Los archivos nativos de vídeo/audio conservan sus bytes y cada salida vuelve a comprobar permisos de carpetas y red. Una generación rechazada por el proveedor es un fallo, no una descarga recuperable; se mantiene la reserva de presupuesto.","Guarda y bloquea versiones de apariencia y voz aprobadas por el usuario. Reutiliza referencias inmutables con hashes desde UI y agentes; bloquea vínculos incompatibles y sustituciones silenciosas antes de generar con pago. Arrastra personajes aprobados entre workspaces con todas sus referencias visuales y de voz, copias locales independientes y nodos nativos agrupados; no se transfieren credenciales ni permisos. Revisa imágenes maestras y voz, asigna entradas compatibles automáticamente e inserta referencias con nombre sin descartar archivos ni cambiar identidades silenciosamente.","Exigir una bóveda protegida por el sistema operativo y conservar las credenciales existentes si el archivo es ilegible o está dañado.","Encuentra skills seleccionadas por nombres con espacios o guiones incluso cuando el registro en línea no esté disponible.","Mantén bloqueados los ajustes de la cuenta de vídeo durante la carga para que las respuestas tardías no borren cambios ni avisos de validación. Conserva las ediciones y selecciones de los diálogos creativos durante las actualizaciones del workspace.","Añade guiones gráficos nativos con escenas ordenadas, diálogo, personajes aprobados, referencias y borradores de imagen/video vinculados. Comparte revisión con CLI/MCP; conserva resultados y muestra dependencias ausentes o desactualizadas sin ejecución paga automática.","Compara variantes de imagen y video con reproducción sincronizada e historial inmutable vinculado al hash. Los archivos modificados invalidan la aprobación; los agentes pueden proponer pero no autoaprobarse.","Añade variación de imagen, eliminación de fondo con alpha real, cambios anotados y borradores de animación. Conserva originales y procedencia congelada en los flujos existentes de imagen Codex y video fal.","Guarda la dirección de cada escena sin perder referencias ni vínculos de voz.","Versiona paletas, logotipos, productos y reglas creativas y coloca los recursos exactos en cualquier workspace.","Guarda guiones versionados con entradas nombradas y recrea storyboards editables sin copiar permisos de ejecución.","Ordena, recorta y subtitula vídeos existentes sin cambiar sus archivos originales.","Elimina políticas, auditoría, eventos de integración y herramientas vinculados al workspace en orden de dependencia, sin dejar el Canvas parcialmente eliminado; conserva otros workspaces y archivos del proyecto.","Actualizar la dependencia de serialización para corregir denegación de servicio por entradas malformadas."]},
    {"date":"16 de septiembre de 2026","title":"Orkestrai 0.31.5: comunicación entre agentes y selección del terminal","summary":"Comunicación fiable entre agentes y selección precisa en el terminal.","items":["Entregar mensajes entre agentes sin esperar toda la respuesta del modelo. Los mensajes recíprocos desbloquean al remitente, los mensajes recibidos por Claude durante el trabajo mantienen su vínculo con el transcript y los avisos válidos de finalización siguen disponibles para el líder.","Alinear las coordenadas del ratón con el zoom del Canvas al arrastrar, seleccionar palabras y utilizar pantallas con escala."]},
    {
      ...
      "items": [
        ...
        "Permite cancelar la apertura del micrófono o la transcripción desde el mismo control en Canvas y Workbench. Detiene la captura inmediatamente y descarta resultados tardíos después de cancelar o navegar. La apertura tiene un límite de 15 segundos y la transcripción de tres minutos; la grabación termina automáticamente a los 15 minutos.",
    ...
    },
    {
      ...
      "summary": "Vuelve a copiar y mover selecciones entre workspaces.",
      "items": [
        "Restaura la copia y el movimiento de selecciones del Canvas entre workspaces, incluidas selecciones grandes de 59 nodos, conservando conexiones internas y posiciones relativas. Las solicitudes no válidas se rechazan antes de modificar nodos, con un mensaje de validación más claro.",
        "Restaura el arrastre de archivos externos: las imágenes soltadas en el espacio libre del Canvas se convierten en nodos de referencia persistentes separados; los archivos soltados sobre un agente se adjuntan al prompt sin enviarlo automáticamente. Reconoce el arrastre protegido antes de soltar y conserva las entradas durante la carga.",
        "Guarda las ediciones numéricas de Diseño una sola vez al pulsar Enter y espera a que termine el guardado antes de aplicar el layout, evitando conflictos de revisión duplicados."
      ]
    },
    {
    ...
    {
      "date": "16 de septiembre de 2026 · 0.31.0",
      "title": "Orkestrai 0.31.0: asistentes continuos en el escritorio",
      "summary": "Seguimiento de conversaciones, memoria privada y trabajo programado con acceso controlado por el usuario.",
      "items": [
        "Supervisa conversaciones autorizadas del escritorio en segundo plano y activa al agente del Canvas cuando lleguen mensajes. En macOS, la navegación automática opcional vuelve a abrir el contacto aprobado.",
        "Procesa mensajes recibidos en lotes persistentes, incluso después de reabrir el chat, con verificación del texto completo, protección contra envíos duplicados y recuperación de escritura interrumpida.",
        "Configura la persona, el idioma y la voz de cada asistente, con memoria privada opcional de la conversación, preferencias con fuentes, controles de retención y eliminación.",
