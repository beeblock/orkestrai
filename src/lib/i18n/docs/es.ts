...
Header: Authorization = Bearer {{accessToken}}`,
    ...
    },
    {
      ...
      body: `Abre el menú Imagen en la barra del Canvas y elige Generar imágenes. El flujo forma parte del canvas existente: conecta Notas para paleta, reglas del personaje, dirección de arte, restricciones o contexto de campaña; conecta hasta cinco referencias ordenadas de Imagen PNG, JPEG o WebP; y conecta un agente Codex activo como ejecutor. Se requiere una cuenta o suscripción de Codex autenticada con ImageGen disponible; no se necesita una clave de API de OpenAI. Las miniaturas numeradas muestran el orden exacto. Escribe el prompt, elige de uno a diez resultados, solicita opcionalmente un fondo PNG realmente transparente y define una carpeta relativa del workspace (generated/images por defecto) con un prefijo seguro. Generar delega un único run lógico a esa sesión de Codex. El agente produce y valida cada resultado por separado; cuando se exige alpha real y ImageGen devuelve un PNG opaco o un cuadriculado dibujado, edita automáticamente ese resultado para quitar el fondo y reintenta hasta tres veces antes de detenerse. Orkestrai nunca solicita ni almacena una clave de API de imagen, nunca llama directamente a un endpoint del provider y no inventa controles de modelo, calidad, tamaño o formato que la tool nativa no expone. Un Codex conectado tiene control tipado completo de este flujo visible mediante image_workflow_*: puede crear y configurar un borrador sin ejecutarlo, conectar, desconectar o reordenar Notas e Imágenes, agregar archivos de referencia del workspace, asumir y ejecutar la generación, validar cada resultado, completar o cancelar y eliminar el flujo. El ejecutor copia cada resultado nativo al destino preasignado en el workspace y llama a image_workflow_validate antes de image_workflow_complete. Orkestrai solo acepta ese Codex conectado y esas rutas exactas, decodifica cada PNG con límites seguros de dimensiones y verifica píxeles realmente transparentes cuando se solicitan antes de crear nodos Imagen conectados con hash de inputs, flujo de origen, id de ejecución, índice del resultado, duración e historial limitado. Los resultados pueden alimentar otro flujo como referencias. Cancelar invalida el run activo para impedir que se materialice una finalización tardía; las ejecuciones abandonadas también expiran tras un tiempo limitado. Canvas, Workbench, CLI y MCP operan el mismo grafo persistido sin un estado paralelo de automatización.`,
      bullets: [
        ...
        'La eliminación del fondo siempre es otra edición nativa de image_gen.imagegen que usa solo el resultado rechazado como referencia y el prompt dedicado del validador. Con múltiples referencias de imagen, ImageGen primero compone el primer plano sobre un fondo blanco uniforme y elimina ese fondo en una edición nativa separada. El feedback de máquina identifica después cualquier fondo incrustado como píxeles opacos y escala el prompt de recorte si la corrección sigue opaca. Se prohíben Python, Pillow, ImageMagick, ffmpeg, remove-bg, máscaras generadas y el procesamiento local de píxeles.',
      ],
    },
    {
    ...
    },
    {
      ...
      body: `Agrega Grafo de Código desde la barra de Canvas para indexar los repositorios aprobados del workspace sin ejecutar código del proyecto, scripts de build, plugins ni configuraciones. El mismo nodo persistente se abre en Workbench y visualiza módulos, imports, clases, interfaces, funciones, métodos, llamadas, instancias, herencia e implementaciones en TypeScript, JavaScript, Svelte y PHP. Elige un repositorio o todo el workspace, busca nombres, rutas, firmas y documentación cercana e inspecciona relaciones entrantes, salientes o bidireccionales hasta una profundidad limitada. Abre Cambios para combinar el working tree Git actual con todos los Floors activos: los archivos cambiados se mapean a símbolos afectados directa y transitivamente, tests probables y conflictos de impacto compartido antes de revisar o integrar. Abre Contratos para cruzar endpoints del backend, llamadas HTTP del frontend, clientes generados, schemas OpenAPI o Swagger, prefijos de gateway y requests vivos del Cliente de API entre repositorios aprobados. La vista destaca requests sin correspondencia y conflictos de ruta entre proyectos; hosts, valores de query, headers y credenciales nunca se guardan en el grafo. Abre Calidad para ver evidencia con confianza explícita sobre duplicación estructural, ciclos de imports, acoplamiento, violaciones inferidas entre capas, código excesivo, ejecución sensible para la seguridad y posible código muerto. Los nodos de recursos del flujo de datos estático muestran nombres de entorno, archivos relativos, rutas de red, tablas de base de datos y canales IPC sin guardar valores, cuerpos, hosts, query strings ni cuerpos de código. Trata cada hallazgo como evidencia para revisión, no como veredicto automático. Las acciones junto a cada alcance de cambios crean un handoff limitado en Kanban; el working tree principal también puede crear una revisión en Review Center vinculada a la revisión Git actual. Los archivos fuente siguen siendo la verdad: Orkestrai guarda hashes, spans, metadatos limitados y relaciones en su propio adapter de grafo versionado sobre SQLite, cambia revisiones de forma atómica y marca el índice como desactualizado cuando cambian archivos compatibles. Los repositorios hermanos autorizados en Editar workspace se convierten en proyectos separados. Los agentes consultan exactamente el mismo grafo visible mediante code_graph_status/index/search/symbol/neighbors/changes/contracts/quality/handoff o los comandos orkestrai graph equivalentes; nunca se exponen SQL ni Cypher arbitrarios. SQLite es el valor local-first detrás de CodeGraphStore y los gates de benchmark permiten un adapter Memgraph futuro sin cambiar la UI ni el contrato de agentes.`,
    },
    {
      id: 'cli',
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Inicia el caso de uso guiado para agregar un nodo Grafo de Código e indexar todos los repositorios aprobados explícitamente para el workspace. Busca un servicio, componente, ruta o método, selecciona el resultado y alterna entre quién lo llama y sus dependencias antes de abrir el código relevante. Abre Contratos para conectar endpoints del backend, llamadas del frontend, clientes generados, schemas OpenAPI, prefijos de gateway y requests vivos del Cliente de API, y revisa llamadas sin correspondencia o conflictos de ruta entre proyectos sin exponer secretos. Abre Calidad para revisar duplicados, ciclos, acoplamiento, límites entre capas, smells, ejecución sensible, posible código muerto y flujo de datos estático con confianza explícita y sin valores secretos almacenados. Abre Cambios para inspeccionar el working tree actual y los Floors activos, incluidos símbolos afectados, tests probables y conflictos entre Floors, y usa las acciones del alcance para crear una revisión en Review Center o tarea Kanban. Un líder o especialista usa el mismo grafo, contratos, evidencia de calidad y handoff limitados por MCP sin cargar repositorios completos en la conversación.',
      tags: ['Inteligencia de código', 'arquitectura e impacto', 'múltiples repositorios'],
    },
    {
