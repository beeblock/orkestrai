...
Header: Authorization = Bearer {{accessToken}}`,
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Agrega un Director Creativo Codex, una nota Brief Visual y un flujo Generar imágenes. Usa una cuenta o suscripción de Codex autenticada con ImageGen disponible; no se necesita una clave de API de OpenAI. Puedes configurar el grafo manualmente o pedir al Director Creativo conectado que cree el flujo, escriba o revise la dirección, agregue y ordene referencias del workspace, solicite hasta diez resultados, ejecute el run completo y devuelva cada resultado validado al canvas. El agente usa su tool nativa image_gen.imagegen con referenced_image_paths, copia cada resultado al destino asignado y completa una ejecución trazable. Las poses aprobadas pueden alimentar la siguiente rama sin perder contexto del prompt ni procedencia. Un líder que no sea Codex puede coordinar el trabajo, pero debe conectar o delegar la ejecución a Codex.',
