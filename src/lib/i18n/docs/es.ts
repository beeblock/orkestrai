...
Header: Authorization = Bearer {{accessToken}}`,
    ...
    },
    {
      ...
      body: `Agrega Tool Workshop desde la barra del Canvas, abre el mismo nodo a pantalla completa desde Herramientas en Workbench o selecciona Herramientas dentro de Automatizaciones. Crea una herramienta versionada del workspace desde una integración conectada, una llamada HTTP limitada, una transformación determinista o un comando del workspace con array de argumentos y sin expansión de shell. Cada manifiesto declara schemas JSON soportados de entrada y salida, capacidades, timeout, límite de salida y fixtures de prueba. La prueba seca valida el borrador actual sin ejecutar su efecto. Solo el dueño del workspace publica una revisión inmutable; los agentes con una tarea activa asignada pueden proponer o actualizar borradores y ejecutar la versión aprobada mediante tool_list, tool_propose, tool_update y tool_execute, pero no pueden activar su propio código. Un borrador posterior nunca reemplaza la revisión activa sin publicación explícita, y rollback crea otro borrador revisable. Las credenciales usan solo SecretRefs de Automatizaciones -> Seguridad y deben estar vinculadas a Tool Workshop, a tool:<slug> y al host exacto o al destino workspace-command antes de publicar. El ejecutor confiable resuelve valores solo en un header HTTP o en el stdin protegido del comando; se rechazan credenciales crudas en el manifiesto y los inputs, mientras inputs, outputs, errores, actividad del Centro de Control y auditoría encadenada se guardan saneados. Usa una clave de idempotencia estable en automatizaciones para que la recuperación no repita un efecto externo. Los flujos existentes de Gmail, Slack, Telegram, WhatsApp, GitHub, Portal, Equipo, PDF y archivos del workspace se pueden componer como herramientas sin crear otro modelo de grafo.`,
    },
    {
      id: 'computer-node',
      title: 'Nodo Equipo y control limitado del escritorio',
      body: `Agrega Computadora desde la barra del Canvas o Herramientas en Workbench. El nodo empieza desactivado e identifica plataforma, pantallas, aplicaciones visibles, ventanas, permiso de accesibilidad y captura. Concede personalmente los permisos del sistema operativo, permite solo los ids de aplicación y pantalla necesarios y luego activa el control. Los agentes necesitan una tarea Kanban activa asignada y usan llamadas tipadas computer_inspect/focus/click/type/type_secret/shortcut/screenshot/wait con claves de idempotencia estables; los mismos ids de aplicación también deben estar en la política permanente de Seguridad del workspace. Los clics y capturas de agentes permanecen dentro de una ventana permitida; las acciones manuales de pantalla completa requieren seleccionar cada pantalla afectada. Una SecretRef de login debe estar vinculada a computer.type_secret y al id exacto de la app; su valor solo se resuelve dentro del ejecutor y llega a macOS o Windows por un canal stdin protegido, no como argumento del proceso. El adaptador Linux X11 actual rechaza la escritura de secretos; allí usa una integración API o SecretRef del Portal Gestionado. Las capturas quedan confinadas en .orkestrai/computer/evidence con retención limitada y sirven como mapa de clic en el nodo visible. Centro de Control y la auditoría encadenada guardan actor, operación, destino, resultado y metadatos seguros de evidencia; el contenido escrito aparece solo como longitud y digest. macOS usa Accesibilidad y Grabación de pantalla nativas, Windows usa el escritorio interactivo y Linux actualmente soporta control X11 sin secretos con xdotool/wmctrl y gnome-screenshot o scrot; las sesiones Wayland incompatibles fallan explícitamente.`,
    },
    {
      id: 'portal',
      title: 'Portal (navegador de los agentes)',
      body: `El nodo Portal es un navegador embebido. Dale a cada Portal un nombre persistente con el lápiz del encabezado; la dirección queda en la barra de navegación separada. Los agentes listan todos los Portales del workspace con nombre, URL, id y estado de conexión y eligen por nombre único o id: orkestrai portal <nombre-o-nodeId> navigate (abrir URL), eval (correr JS en la página), dom (leer el HTML), screenshot. Un Portal no conectado todavía existe y debe reutilizarse; crear otro exige intención explícita. Conectado a un agente, se vuelve los ojos del agente. Úsalo para probar la aplicación que el equipo está construyendo (apunta el portal al dev server) o investigar en la web. En la app de escritorio, los enlaces comunes que solicitan una pestaña en primer o segundo plano crean otro nodo Portal en el mismo Canvas. Los pop-ups reales de login y pago permanecen en una ventana aislada de Orkestrai, no en el navegador del sistema, conservando window.opener y la misma sesión. Las cookies persistentes y el almacenamiento web se escriben en disco, y el nodo restaura la última URL navegada después de reiniciar; los sitios aún pueden usar cookies de sesión que expiran al cerrar. La automatización completa corre en Electron; en un navegador común el portal es solo visor. El ícono de teléfono en el encabezado del Portal abre una barra de responsividad, parecida al device toolbar de un navegador: elige un dispositivo (iPhone, Pixel, iPad, laptop, desktop) o escribe un ancho/alto exacto, rota la orientación, o desactívala para volver a llenar el nodo. El viewport real de la página cambia a ese tamaño exacto — igual que redimensionar una ventana real — así que su propio CSS responsivo reacciona con normalidad; si el tamaño emulado es más grande que el nodo, el Portal se desplaza hasta ahí en vez de encoger o distorsionar la página.`,
    },
    {
      id: 'portal-new-tabs',
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Agrega un nodo Equipo, concede los permisos necesarios del sistema, permite solo la app y las pantallas destino y activa el mismo id de app en Automatizaciones -> Seguridad. Conecta un agente Persistente con una tarea Kanban activa. Puede inspeccionar ventanas, enfocar la app permitida, hacer clic con coordenadas normalizadas o absolutas, escribir, usar atajos limitados, esperar el estado de una ventana y capturar evidencia. Cada acción intermediada es idempotente y auditada, sin guardar texto escrito ni credenciales. Prefiere el Centro de integraciones o el Portal administrado cuando estén disponibles; usa Equipo para herramientas instaladas sin API ni flujo web.',
  ...
  changelog: [
    {
      date: 'En desarrollo · Próxima versión minor',
      title: 'Core de Orkestrai: la base del trabajo autónomo 24/7',
      summary: 'El Core de ejecución puede seguir después de cerrar las ventanas, iniciar con el sistema, informar su estado y recuperarse tras fallos o suspensión.',
      items: [
