      ...
      <div class="space-y-2 border-l-2 border-[var(--app-accent)] pl-3 text-xs" data-testid="character-binding">
        ...
        <details open={binding.imagePointers.some(path => !path) || !binding.voicePointer}>
          ...
          <div class="space-y-2 py-2">
            ...
            {#each binding.imagePointers as path, position}<div class="space-y-1"><span>{m['creative.character_image_input']({ index: String(position + 1) })}</span><ModelCombobox value={path} options={slots.filter(path => /image|frame/i.test(path)).map(path => ({ value: path, label: path }))} defaultLabel={m['creative.media_pointer']()} searchPlaceholder={m['creative.media_pointer']()} emptyLabel={m['creative.no_inputs']()} ariaLabel={m['creative.character_image_input']({ index: String(position + 1) })} onValueChange={(path) => pointer(index, position, path)} /></div>{/each}
            <div class="space-y-1"><span>{m['creative.character_voice_input']()}</span><ModelCombobox value={binding.voicePointer} options={(record?.definition.voice.kind === 'provider' ? voiceIds : slots.filter(path => /audio|voice/i.test(path))).map(path => ({ value: path, label: path }))} defaultLabel={m['creative.media_pointer']()} searchPlaceholder={m['creative.media_pointer']()} emptyLabel={m['creative.no_inputs']()} ariaLabel={m['creative.character_voice_input']()} onValueChange={(path) => pointer(index, 'voice', path)} /></div>
