...
<section class={`flex h-full min-h-0 flex-col bg-[var(--app-sidebar)] text-[var(--app-text)] ${compact ? 'w-[min(430px,42vw)] border-l border-[var(--app-border)]' : 'w-full'}`} data-testid="automation-workspace">
  ...
  <Tabs.Root bind:value={activeTab} class="grid min-h-0 flex-1 grid-rows-[38px_minmax(0,1fr)]">
    ...
    <Tabs.Content value="overview" class="m-0 min-h-0 overflow-y-auto p-4">
      ...
      {#if editorOpen}
        <form method="POST" use:enhance class="mb-4 border-l-2 border-[var(--app-accent)] bg-[var(--app-surface)] p-4">
          ...
          <div class="mt-4 flex items-center justify-between gap-3"><label class="flex items-center gap-2 text-ui-xs"><Switch checked={$formData.enabled} onCheckedChange={(checked) => ($formData.enabled = checked)} />{m['automation.enable']()}</label><div class="flex gap-2"><Button type="button" variant="ghost" size="sm" onclick={resetEditor}>{m['automation.cancel']()}</Button><Button type="submit" size="sm" disabled={busy}>{#if busy}<LoaderCircle class="animate-spin" />{/if}{m['automation.save']()}</Button></div></div>
        </form>
      {/if}
    ...
    <Tabs.Content value="history" class="m-0 min-h-0 overflow-y-auto p-4">{#if runs.length === 0}<div class="grid min-h-52 place-items-center text-center"><div><History class="mx-auto text-[var(--app-text-muted)]" size={24} /><p class="mt-2 text-xs">{m['automation.history_empty']()}</p></div></div>{:else}<div class="divide-y divide-[var(--app-border)] border-y border-[var(--app-border)]">{#each runs as run (run.id)}<article class="flex items-start gap-3 bg-[var(--app-surface)] px-3 py-3">{#if run.status === 'succeeded'}<CheckCircle2 class="mt-0.5 shrink-0 text-[var(--app-success)]" size={15} />{:else if run.status === 'failed'}<XCircle class="mt-0.5 shrink-0 text-[var(--app-danger)]" size={15} />{:else}<LoaderCircle class={`mt-0.5 shrink-0 text-[var(--app-accent)] ${run.status === 'running' ? 'animate-spin' : ''}`} size={15} />{/if}<div class="min-w-0 flex-1"><div class="flex flex-wrap items-center gap-2"><span class="text-xs font-medium">{automations.find((item) => item.id === run.routineId)?.name ?? run.routineId.slice(0, 8)}</span><Badge variant="outline">{statusLabel(run.status)}</Badge></div><p class="mt-1 break-words text-ui-xs leading-4 text-[var(--app-text-soft)]">{run.detail ?? run.error}</p><p class="mt-1 text-ui-xs text-[var(--app-text-muted)]">{new Date(run.ranAt).toLocaleString()} · {m['automation.attempt']({ attempt: run.attempt })}{run.durationMs !== null ? ` · ${m['automation.duration']({ duration: run.durationMs })}` : ''}{run.provider ? ` · ${run.provider}` : ''}</p></div>{#if run.recoverable && run.attempt < 3}<Button variant="ghost" size="sm" onclick={() => retry(run)}><RotateCcw size={13} />{m['automation.retry']()}</Button>{/if}</article>{/each}</div>{/if}</Tabs.Content>
