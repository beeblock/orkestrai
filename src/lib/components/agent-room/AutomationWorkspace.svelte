          ...
          <div class={`grid gap-3 ${compact ? 'grid-cols-1' : 'grid-cols-2'}`}>
            ...
            {#if $formData.triggerType === 'webhook'}
              <div class={compact ? '' : 'col-span-2'}>
                <label><span class="mb-1 block text-ui-xs font-medium">{m['automation.webhook_secret']()}</span><Input type="password" bind:value={$formData.webhookSecret} autocomplete="new-password" /></label>
                <p class="mt-1 text-ui-xs text-[var(--app-text-muted)]">{m['automation.webhook_hint']()}</p>
                {#if editingId}
                  <code class="mt-2 block select-all overflow-x-auto border border-[var(--app-border)] bg-[var(--app-canvas)] p-2 text-ui-xs">{typeof window !== 'undefined' ? window.location.origin : ''}/api/agent-room/workspaces/{workspaceId}/automations/webhook/{editingId}</code>
                  <p class="mt-2 text-ui-xs leading-5 text-[var(--app-text-muted)]">{m['automation.webhook_local']()}</p>
                {/if}
              </div>
            {/if}
