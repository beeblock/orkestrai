<script lang="ts">
  ...
  let { schema, value, onChange, onValidityChange }: { schema: ModelSchema; value: Record<string, unknown>; onChange: (value: Record<string, unknown>) => void; onValidityChange: (valid: boolean) => void } = $props();
...
{#snippet field(name: string, original: ModelSchema)}
  ...
  <div class="min-w-0 space-y-1.5 text-xs">
    ...
    {#if choices && choices.every(item => ['string', 'number', 'boolean'].includes(typeof item))}
