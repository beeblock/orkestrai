<script lang="ts">
  ...
  let confirmProvider = $state(false), pendingModel = $state('');
  ...
  async function loadContract(modelId: string, defaults = false) {
    ...
    try {
      const next = await creativeApi<FalModelContract>(`${base}/models?provider=${provider}&endpoint=${encodeURIComponent(modelId)}`);
      if (sequence !== contractSequence || config.modelId !== modelId || config.provider !== provider) return;
      contract = next;
  ...
  async function refresh(reset = false) {
    ...
    try {
      ...
      if (reset || !dirty) {
        ...
        const changedModel = next.modelId !== config.modelId || next.provider !== config.provider;
        config = next;
        if (reset || changedModel) { catalogProvider = next.provider; void loadCatalog(); }
  ...
  async function chooseModel(value: string, confirmed = false) {
    if (!value || (value === config.modelId && !providerPending)) return;
    try {
      config = switchCreativeModel($state.snapshot(config), value, catalogProvider, confirmed);
    } catch (cause) {
      if (providerPending && cause instanceof Error && cause.message === 'creative_model_mapping_required') { pendingModel = value; confirmProvider = true; return; }
      error = cause instanceof Error && cause.message === 'creative_model_mapping_required' ? cause.message : 'creative_unsupported_input'; return;
    }
    billing = null;
  ...
  async function execute(action: 'save' | 'estimate' | 'run') {
    if (providerPending) { error = 'creative_provider_model_required'; return; }
    busy = true; error = '';
    try {
      if (action !== 'run') { await persist(); if (action === 'estimate') preview = await creativeApi<CreativePreview>(`${endpoint}/preview`, 'POST', {}, 300000); }
...
<AlertDialog.Root bind:open={confirmProvider}>
  <AlertDialog.Content>
    <AlertDialog.Header><AlertDialog.Title>{m['creative.provider_switch_title']()}</AlertDialog.Title><AlertDialog.Description>{m['creative.provider_switch_help']()}</AlertDialog.Description></AlertDialog.Header>
    <AlertDialog.Footer><AlertDialog.Cancel>{m['dlg.cancel']()}</AlertDialog.Cancel><AlertDialog.Action onclick={() => { confirmProvider = false; void chooseModel(pendingModel, true); }}>{m['creative.provider_switch_confirm']()}</AlertDialog.Action></AlertDialog.Footer>
  </AlertDialog.Content>
</AlertDialog.Root>
