                  ...
                  <div class="overflow-hidden rounded-lg shadow-[var(--app-shadow-border)]">
                    {#each snapshot?.agents ?? [] as agent (agent.id)}
                      <div class="agent-row" class:selected={selectedAgents.includes(agent.id)}>
                        ...
                        <div class="min-w-0"><div class="flex items-center gap-1.5"><span class="truncate text-ui-lg font-medium">{agent.title}</span>{#if agent.maestro}<Badge variant="outline">{m['council.leader']()}</Badge>{/if}</div><p class="mt-0.5 truncate text-ui-sm text-[var(--app-text-muted)]">{agent.provider}{agent.model ? ` · ${agent.model}` : ''}{agent.role ? ` · ${agent.role}` : ''}</p><p class="mt-0.5 truncate font-mono text-[10.5px] tabular-nums text-[var(--app-text-muted)]">{usageSummary(agent.provider)}</p></div>
