    ...
    <div class="grid min-h-0 flex-1 auto-rows-[minmax(430px,1fr)] grid-cols-1 gap-4 overflow-y-auto overscroll-contain sm:grid-cols-2">
      {#each selections as nodeId, index (index)}
        ...
        <section class="flex min-h-0 min-w-0 flex-col gap-2" aria-label={index === 0 ? 'A' : 'B'}>
          ...
          {#if inspection}<details class="group shrink-0 text-ui-md"><summary class="flex cursor-pointer list-none items-center gap-1.5 text-[var(--app-text-muted)] hover:text-[var(--app-text)] [&::-webkit-details-marker]:hidden"><ChevronRight size={13} class="shrink-0 transition-transform duration-150 group-open:rotate-90" aria-hidden="true" />{m['creative_review.history']()} <span class="font-mono text-[11px] tabular-nums">({inspection.history.length})</span></summary><div class="grid max-h-28 gap-2 overflow-y-auto py-2 pl-5"><p class="font-mono text-[11px] break-all text-[var(--app-text-muted)]">{inspection.snapshot.media.path}</p>{#each inspection.snapshot.characters as character}<p>{character.name} · v{character.version}</p>{/each}{#each inspection.history as review}<p class="break-words"><strong class="font-medium">{decisionLabel(review.decision)}</strong> · {new Date(review.createdAt).toLocaleString()} · {review.actorType === 'user' ? m['creative_review.owner']() : m['creative_review.agent']()}<br />{review.comment}<br /><code class="font-mono text-[11px] break-all text-[var(--app-text-muted)]">{review.snapshot.media.sha256}</code></p>{/each}</div></details>{/if}
        </section>
      {/each}
    </div>
