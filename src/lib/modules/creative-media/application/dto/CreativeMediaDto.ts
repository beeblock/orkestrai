...
export type CreativeCommand = 'characters' | 'models' | 'list' | 'read' | 'create' | 'update' | 'preview' | 'run' | 'cancel' | 'retry_download' | 'close_unconfirmed' | 'remove';
export class CreativeMediaDto {
  ...
  static from(workspaceId: string, actor: CreativeActor, command: CreativeCommand, nodeId?: string, input?: unknown, runId?: string) {
    const value = command === 'characters' ? creativeCharacterCommandSchema.parse(input) : command === 'models' ? creativeCatalogQuerySchema.parse(input ?? {}) : command === 'create' || command === 'update' ? creativeWorkflowSaveSchema.parse(input)
    ...
    if (!['characters', 'models', 'list', 'create', 'cancel', 'retry_download', 'close_unconfirmed'].includes(command) && !nodeId) throw new CreativeMediaError('creative_node_required');
