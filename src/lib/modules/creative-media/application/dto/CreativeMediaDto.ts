...
import { creativeRunCommandSchema, creativeRunRequestSchema, creativeWorkflowSaveSchema } from '../../contracts/schemas/creative-media.schema.js';
export type CreativeCommand = 'list' | 'read' | 'create' | 'update' | 'preview' | 'run' | 'cancel' | 'retry_download' | 'close_unconfirmed' | 'remove';
export class CreativeMediaDto {
  ...
  static from(workspaceId: string, actor: CreativeActor, command: CreativeCommand, nodeId?: string, input?: unknown, runId?: string) {
    const value = command === 'create' || command === 'update' ? creativeWorkflowSaveSchema.parse(input)
    ...
    if (!['list', 'create', 'cancel', 'retry_download', 'close_unconfirmed'].includes(command) && !nodeId) throw new CreativeMediaError('creative_node_required');
