...
- Tools MCP \`computer_inspect/focus/click/type/type_secret/shortcut/screenshot/wait\` (ou \`orkestrai computer ...\`) — controlam o nó Computador compartilhado com a UI. Leia \`computer_inspect\` primeiro; toda ação exige nó ativado, app/monitor permitido, tarefa Kanban ativa atribuída a você e chave de idempotência estável. Use \`type_secret\` somente com uma SecretRef listada pelo inspect e vinculada a \`computer.type_secret\` e ao app exato; o valor nunca retorna ao agente. A política de Segurança continua sendo o limite final. Capturas ficam confinadas ao workspace e a auditoria nunca persiste o texto digitado. Não tente contornar o contrato com AppleScript, PowerShell, xdotool ou outra automação de desktop pelo shell.
...
`;
  ...
  private agentsMdBlock(): string {
    return [
      ...
      '- `tool_list/propose/update/execute` / `orkestrai tool ...` — propõe e usa ferramentas reutilizáveis do workspace. Agentes só alteram rascunhos versionados e executam a revisão publicada; somente o usuário publica, arquiva ou restaura. Toda mutação exige task ativa e chave idempotente. Contratos, capacidades, limites e fixtures são obrigatórios; credenciais só entram como SecretRefs e nunca retornam ao agente.',
      ...
      '- `computer_inspect/focus/click/type/type_secret/shortcut/screenshot/wait` / `orkestrai computer ...` — controla o nó Computador compartilhado com a UI. Leia inspect primeiro; ações exigem nó ativado, app/monitor permitido, tarefa Kanban ativa atribuída e chave idempotente. type_secret aceita somente SecretRef listada e ligada a computer.type_secret + app exato; o valor não retorna ao agente. Segurança continua sendo o limite final; capturas ficam no workspace e o texto digitado não entra na auditoria. Nunca contorne o contrato com automação desktop pelo shell.',
