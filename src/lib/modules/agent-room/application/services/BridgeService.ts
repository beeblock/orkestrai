...
- Tools MCP \`code_graph_status/index/search/symbol/neighbors/changes/handoff\` (ou \`orkestrai graph ...\`) — consulte o grafo nativo compartilhado antes de explicar arquitetura, dependências ou impacto. Confira o status, indexe sob demanda se estiver idle/stale, pesquise o símbolo primeiro, use somente travessias limitadas e consulte \`changes\` antes de revisar ou integrar trabalho. Use \`handoff\` para transformar um escopo confirmado em review do working tree principal ou tarefa Kanban rastreável; Floors e repositórios irmãos viram tarefa até serem integrados. Nunca invente relações ausentes nem peça SQL/Cypher arbitrário.
...
`;
  ...
  private agentsMdBlock(): string {
    return [
      ...
      '- `code_graph_status/index/search/symbol/neighbors/changes/handoff` / `orkestrai graph ...` — consulta o mesmo grafo nativo visível no Canvas e Workbench; confira status, indexe sob demanda, pesquise o símbolo, use travessias limitadas e consulte `changes` antes de revisar ou integrar trabalho. Use `handoff` para transformar um escopo confirmado em review do working tree principal ou tarefa Kanban rastreável; Floors e repositórios irmãos viram tarefa até serem integrados.',
