...
- Native video: use \`video_workflow_list/read/create/update/preview/run/cancel/retry_download/remove\` or \`orkestrai video <command> --task <assignedTaskId> --input '<json>'\`. List returns authorized account ids, exact model capabilities and Image/Note ids. Create/update requires {title,config,revision?}; preview returns id/revision/cost and outbound references. Run requires {revision,previewId,idempotencyKey}; reuse the SAME UUID after a timeout. Read returns persisted status and workspace MP4 output. Only declared startImageNodeId/endImageNodeId/contextNodeIds supply data; ordinary edges do not trigger execution. The owner must configure the fal key in the desktop vault, workspace opt-in, models, budget, external-data and agent permission. Never request a key in chat, call fal directly, retry uncertain submissions, or claim queued work is delivered. Retry_download never generates again. Codex image workflows remain separate and unchanged, without an API key.
