...
`;
  ...
  private agentsMdBlock(): string {
    return [
      ...
      '- `image_workflow_*` — controle completo dos fluxos nativos de imagem executados por Codex: crie/configure sem rodar, conecte ou reordene Notas e Imagens, adicione referencias do workspace, gere de 1 a 10 outputs, conclua, cancele ou remova. Leia o contrato, use somente `image_gen.imagegen` com `referenced_image_paths`, copie os outputs para os destinos do workspace e confirme com `image_workflow_complete`. Nunca peça chave de API nem use API/script paralelo.',
