...
Se as tools \`orkestrai\` (list/usage/ask/huddle_*/memory_*/note_*/api_client_*/image_workflow_*/design_*/task_*/portal_*/floor_*/device_*/notify/port/recruit/dismiss) estiverem disponíveis como MCP neste ambiente, PREFIRA elas (chamadas tipadas, sem parse de shell) — a CLI continua valendo como fallback.
...
`;
  ...
  private agentsMdBlock(): string {
    return [
      ...
      '- `image_workflow_*` — controle completo dos fluxos nativos de imagem executados por Codex: crie/configure sem rodar, conecte ou reordene Notas e Imagens, adicione referencias do workspace, gere de 1 a 10 outputs, valide, conclua, cancele ou remova. Leia o contrato, use somente `image_gen.imagegen` com `referenced_image_paths`, copie cada output para o destino do workspace e chame `image_workflow_validate`. Se alpha real for exigido e falhar, edite a propria saida para remover o fundo e valide novamente, no maximo tres tentativas. Chame `image_workflow_complete` apenas quando todos validarem. Nunca peça chave de API nem use API/script paralelo.',
