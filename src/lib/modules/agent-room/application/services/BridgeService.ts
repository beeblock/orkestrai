...
`;
  ...
  private agentsMdBlock(): string {
    return [
      ...
      '- `code_graph_status/index/search/symbol/neighbors/changes` / `orkestrai graph ...` — consulta o mesmo grafo nativo visível no Canvas e Workbench; confira status, indexe sob demanda, pesquise o símbolo, use travessias limitadas e consulte `changes` antes de revisar ou integrar trabalho para mapear arquivos alterados, impacto, testes prováveis e conflitos entre Floors.',
