...
- Native video: use \`video_workflow_models/list/read/create/update/preview/run/cancel/retry_download/remove\` or \`orkestrai video <command> --task <assignedTaskId> --input '<json>'\`. List returns authorized account ids, exact model capabilities and Image/Video/Note ids. Create/update requires {title,config,revision?}; preview returns id/revision/cost and outbound references. Run requires {revision,previewId,idempotencyKey}; reuse the SAME UUID after a timeout. Read returns persisted status and native video/audio outputs in the workspace (up to 10 files, without transcoding). Use models to discover ALL fal video endpoints, then request input.endpoint to read the exact contract. Generic endpoints use config.parameters plus mediaBindings (JSON pointer and Image/Video nodeId or workspace-relative image/audio/video path). Never assume Wan/Kling parameters apply to Seedance, Veo, Luma, MiniMax, Runway or other families. Only declared mediaBindings/startImageNodeId/endImageNodeId/contextNodeIds supply data; ordinary edges do not trigger execution. The owner must configure the fal key in the desktop vault, workspace opt-in, models, budget, external-data and agent permission. Never request a key in chat, call fal directly, retry uncertain submissions, or claim queued work is delivered. Retry_download never generates again. Codex image workflows remain separate and unchanged, without an API key.
- Storyboard-to-video: write scene order, prompts, dialogue, language and sound direction in Notes. Create storyboard/reference images only through the existing Codex image workflows and image_gen.imagegen; never use fal to generate those images. Wait for validated image outputs and bind their exact node IDs to the selected video endpoint. Approved references are uploaded only to generate video. When speech or sound is required, verify the model documentation and supported language, enable its documented native-audio field and include dialogue/sound cues in the prompt. Do not silently select a mute model, claim a fixed recurring voice, or confuse generated speech with Orkestrai TTS. Review delivered clips for both picture and sound; a completed job does not prove dialogue accuracy. Multiple clips are separate outputs, not an automatically edited movie. Use video_workflow_characters to create/read/update character drafts, then ask the owner to lock the reviewed version. Bind that exact ID with config.characterBindings for every scene; appearance and voice masters are immutable, hashed workspace assets. Never drop or replace character bindings silently. place adds the approved local identity as grouped native Notes/Images; cross-workspace library import is owner-only and never copies credentials or spending grants.
...
`;
  ...
  private agentsMdBlock(): string {
    return [
      ...
      '- `video_workflow_models/list/read/create/update/preview/run/cancel/retry_download/remove` / `orkestrai video` — native paid fal video workflows in the same Canvas and Workbench. Requires a live authenticated agent and assigned task. Discover the complete fal video catalog with models, read input.endpoint for the exact official schema, then configure parameters and ordered mediaBindings for images/videos/audio. List authorized profiles; bind context Notes explicitly, preview the outbound data/cost, then run with the returned revision/previewId and a stable UUID idempotencyKey. Owner-only vault, model/data/agent grants and budgets apply. Never request a key, bypass policy with raw HTTP, retry uncertain submissions, or claim delivery before completed/output. Codex image generation is unchanged and still requires no API key.',
      '- Storyboard-to-video: plan scene prompts, dialogue, language and sound cues in Notes; use existing Codex image_workflow tools and image_gen.imagegen for all storyboard/reference images, never fal image generation. Bind validated Image outputs to video inputs. For requested speech/sound, inspect model documentation and language support, enable its native-audio control and include the dialogue in the prompt. No silent substitution with mute models, no assumed persistent voice or automatic multi-clip editing. Review picture and sound before claiming delivery. Use video_workflow_characters to create/read/update character drafts, then ask the owner to lock the reviewed version. Bind that exact ID with config.characterBindings for every scene; appearance and voice masters are immutable, hashed workspace assets. Never drop or replace character bindings silently. place adds the approved local identity as grouped native Notes/Images; cross-workspace library import is owner-only and never copies credentials or spending grants.',
