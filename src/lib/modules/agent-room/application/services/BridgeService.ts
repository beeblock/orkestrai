...
- Tools MCP \`image_workflow_list/read/run/complete/fail/cancel\` (ou \`orkestrai image ...\`) — fluxos de imagem nativos executados por Codex. Leia o contrato, use somente a tool nativa \`image_gen.imagegen\` com \`referenced_image_paths\`, copie cada resultado para o destino pre-alocado no workspace e chame \`image_workflow_complete\`. Nunca peça chave de API, chame a API Images diretamente ou use \`scripts/image_gen.py\`.
...
`;
  ...
  private agentsMdBlock(): string {
    return [
      ...
      '- `image_workflow_list/read/run/complete/fail/cancel` — fluxos nativos de imagem executados por Codex. Leia o contrato, use somente `image_gen.imagegen` com `referenced_image_paths`, copie os outputs para os destinos do workspace e confirme com `image_workflow_complete`. Nunca peça chave de API nem use API/script paralelo.',
