...
- Tools MCP \`code_graph_status/index/search/symbol/neighbors/changes/contracts/quality/semantic/evidence/handoff\` (ou \`orkestrai graph ...\`) — consulte o grafo nativo compartilhado antes de explicar arquitetura, dependências ou impacto. Confira o status, indexe sob demanda se estiver idle/stale, pesquise o símbolo primeiro e use somente travessias limitadas. Consulte \`changes\` antes de revisar ou integrar trabalho, \`contracts\` para cruzar endpoints e clientes e \`quality\` para evidências limitadas de arquitetura, qualidade e fluxo de dados sem valores sensíveis. Construa \`semantic\` explicitamente antes de pesquisar por intenção; o índice é local, opcional e fica stale com uma nova revisão. Importe \`evidence\` apenas de um caminho relativo confinado ao repositório aprovado; LCOV, JUnit, traceback e JSON estruturado viram relações derivadas, nunca logs brutos. Achados de qualidade e runtime são evidências com confiança explícita e exigem revisão, não uma afirmação automática de defeito. Use \`handoff\` para transformar um escopo confirmado em review do working tree principal ou tarefa Kanban rastreável; Floors e repositórios irmãos viram tarefa até serem integrados. Nunca invente relações ausentes nem peça SQL/Cypher arbitrário.
...
`;
  ...
  private agentsMdBlock(): string {
    return [
      ...
      '- `code_graph_status/index/search/symbol/neighbors/changes/contracts/quality/semantic/evidence/handoff` / `orkestrai graph ...` — consulta o mesmo grafo nativo visível no Canvas e Workbench; confira status, indexe sob demanda, pesquise o símbolo e use travessias limitadas. Consulte `changes` antes de revisar ou integrar trabalho, `contracts` para cruzar endpoints e clientes e `quality` para evidências limitadas de arquitetura, qualidade e fluxo de dados sem valores sensíveis. Construa `semantic` explicitamente antes de pesquisar por intenção; o índice é local, opcional e fica stale com uma nova revisão. Importe `evidence` apenas de caminho relativo confinado ao repositório aprovado; LCOV, JUnit, traceback e JSON estruturado viram relações derivadas, nunca logs brutos. Achados de qualidade e runtime são evidências com confiança explícita e exigem revisão, não uma afirmação automática de defeito. Use `handoff` para transformar um escopo confirmado em review do working tree principal ou tarefa Kanban rastreável; Floors e repositórios irmãos viram tarefa até serem integrados.',
