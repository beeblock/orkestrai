...
const copy: Record<CreateDesignExplorationDto['locale'], DesignExplorationCopy> = {
  'pt-BR': {
    ...
    tasks: [
      ...
      { kind: 'concept', direction: 'clarity', title: '2A. Conceito A - Clareza', description: 'Entregue este gate inteiro em até 5 minutos: uma tela principal desktop e uma mobile representativas da direção A. Faça uma composição semântica com design_import_code, permita no máximo uma correção de mutação depois da importação e rode a auditoria uma vez. Limite-se a 30-120 layers úteis. Siga literalmente a categoria e o conteúdo do produto. Em 100% de zoom, use hierarquia profissional, texto legível, espaçamento consistente e raios moderados; reserve pills completos a chips e toggles compactos, nunca a botões, banners ou contêineres largos. Não entregue placeholders, labels de debug, decoração arbitrária nem qualquer erro da auditoria. Não crie notas de revisão, catálogo de estados, tokens, componentes, protótipo ou código final neste gate; isso pertence apenas à direção aprovada. Assim que as duas telas estiverem legíveis e sem erros, marque a tarefa pronta para revisão humana.' },
  ...
  en: {
    ...
    tasks: [
      ...
      { kind: 'concept', direction: 'clarity', title: '2A. Concept A - Clarity', description: 'Complete this entire gate within 5 minutes: one representative desktop screen and one mobile screen for direction A. Compose once with semantic HTML/CSS through design_import_code, allow at most one corrective mutation after import, and run the audit once. Stay within 30-120 useful layers. Follow the requested product category and content literally. At 100% zoom, use professional hierarchy, readable copy, consistent spacing, and moderate control radii; reserve full pills for compact chips and toggles, never buttons, banners, or wide containers. Do not deliver placeholders, debug labels, arbitrary decoration, or any remaining audit error. Do not create review notes, state catalogs, tokens, components, prototypes, or final code in this gate; those belong only to the approved direction. As soon as both screens are legible and error-free, mark the task ready for human review.' },
  ...
  es: {
    ...
    tasks: [
      ...
      { kind: 'concept', direction: 'clarity', title: '2A. Concepto A - Claridad', description: 'Completa este gate entero en un máximo de 5 minutos: una pantalla principal desktop y una mobile representativas de la dirección A. Compón una vez con HTML/CSS semántico mediante design_import_code, permite como máximo una mutación correctiva después de importar y ejecuta la auditoría una vez. Limítate a 30-120 capas útiles. Sigue literalmente la categoría y el contenido solicitados. Al 100% de zoom, usa jerarquía profesional, texto legible, espaciado consistente y radios moderados; reserva pills completos para chips y toggles compactos, nunca botones, banners o contenedores anchos. No entregues placeholders, labels de debug, decoración arbitraria ni ningún error pendiente de auditoría. No crees notas de revisión, catálogo de estados, tokens, componentes, prototipo ni código final en este gate; pertenecen solo a la dirección aprobada. Cuando ambas pantallas sean legibles y no tengan errores, marca la tarea lista para revisión humana.' },
